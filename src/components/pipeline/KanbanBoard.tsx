...
export function KanbanBoard({
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="flex h-full gap-4 overflow-x-auto p-8 pt-6">
          ...
          return (
            <div key={status} className="flex w-72 shrink-0 flex-col">
              <div className="mb-3 flex items-center gap-2 px-1">
                ...
                <h3 className="text-sm font-semibold text-ink-700">{LEAD_STATUS_LABELS[status]}</h3>
                <span className="ml-auto rounded-full bg-ink-100 px-2 py-0.5 text-xs font-medium text-ink-500">
                  {columnLeads.length}
                </span>
              </div>
                ...
                {(provided, snapshot) => (
                  <div
                    ...
                    className={cn(
                      ...
                      snapshot.isDraggingOver && 'border-brand-300 bg-brand-50/50',
                    )}
                  >
                      ...
                      <Draggable key={lead.id} draggableId={lead.id} index={index}>
                        {(dragProvided, dragSnapshot) => (
                          <div
                            ...
                            className={cn(
                              'rounded-lg border border-ink-100 bg-white p-3 shadow-[0_1px_2px_rgba(16,24,40,0.04)]',
                              dragSnapshot.isDragging && 'shadow-lg ring-2 ring-brand-200',
                            )}
                          >
                            <Link to={`/crm/${lead.id}`} className="block text-sm font-semibold text-ink-900 hover:text-brand-600">
                              {lead.companyName}
                            </Link>
                            <p className="mt-0.5 truncate text-xs text-ink-400">{lead.responsibleName ?? lead.contactName}</p>
                            <div className="mt-2.5 flex items-center justify-between">
                              <TemperatureBadge temperature={lead.temperature} />
                              <span className="text-sm font-semibold tabular-nums text-ink-900">{lead.score}</span>
                            </div>
                            {lead.nextAction && (
                              <p className="mt-2 truncate rounded-md bg-ink-50 px-2 py-1 text-[11px] text-ink-500">
                                {lead.nextAction}
                              </p>
                            )}
                          </div>
